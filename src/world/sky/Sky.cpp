...
Vec3f Sky::getColor(const Vec3f& direction) const
{
    ...
    float sunAmount = CLAMP(pow(sunAngle, 512.0f) * 1.5f, 0.0f, 1.0f);
    Vec3f sunColor(1.0f, 0.9f, 0.7f);

    return LERP(getColor() * (direction.y + 3.0f) / 3.0f, sunColor, sunAmount);
}
...
{
    return Vec3f(-0.5f, -0.19f, -0.75f).normalized();
}
